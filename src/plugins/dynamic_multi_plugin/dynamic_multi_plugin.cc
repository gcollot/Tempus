...
const DynamicMultiPlugin::OptionDescriptionList DynamicMultiPlugin::option_descriptions()
{
    ...
    odl.declare_option( "Algo/algorithm", "Type of algorithm : label-setting (Dijkstra) (1) or label-correcting (Pape) (2)", 1); 
...
void DynamicMultiPlugin::pre_process( Request& request )
{
    ...
    get_option( "Algo/algorithm", algorithm_ ); 
...
void DynamicMultiPlugin::process()
{
    ...
    try {
        ...
        if ( use_heuristic ) { // Speed-up heuristic
            double h_speed_max;
            get_option( "AStar/speed_heuristic", h_speed_max );

            if ( reversed ) { // Graph reversed
                ...
                else if ( algorithm_ == 2 ) // Label-correcting
                	combined_lc_algorithm_no_init( rgraph, s_.automaton, destination_o, pred_pmap, potential_pmap, cost_calculator, trip_pmap, wait_pmap, shift_pmap, request_.allowed_modes(), rvis, heuristic ); 
            }
            else { // Graph not reversed
                ...
                if ( algorithm_ == 1 ) // Label-setting
                	...
                	combined_lc_algorithm_no_init( graph_, s_.automaton, origin_o, pred_pmap, potential_pmap, cost_calculator, trip_pmap, wait_pmap, shift_pmap, request_.allowed_modes(), vis, heuristic );
            }
        }
        else { // No heuristic
            if ( reversed ) { // Graph reversed
                ...
                if ( algorithm_ == 1 ) // Label-setting
                	...
                	combined_lc_algorithm_no_init( rgraph, s_.automaton, destination_o, pred_pmap, potential_pmap, cost_calculator, trip_pmap, wait_pmap, shift_pmap, request_.allowed_modes(), rvis, NullHeuristic() );
            }
            else { // Graph not reversed
                ...
                if ( algorithm_ == 1 ) // Label-setting
                	...
                	combined_lc_algorithm_no_init( graph_, s_.automaton, origin_o, pred_pmap, potential_pmap, cost_calculator, trip_pmap, wait_pmap, shift_pmap, request_.allowed_modes(), vis, NullHeuristic() );
            }
        }
    }
